...
let
  rootfsImage = pkgs.callPackage ../../../../lib/make-fs.nix ({
    inherit (config.sdImage) storePaths;
    compressImage = config.sdImage.compressImage;
    populateImageCommands = config.sdImage.populateRootCommands;
    volumeLabel = config.sdImage.volumeLabel;
...
in
{
  ...
  options.sdImage = {
    ...
    volumeLabel = mkOption {
      type = types.nullOr types.str;
      default = "NIXOS_SD";
      example = "NiXOS_SD";
      description = lib.mdDoc ''
        Name of the volume which holds the filesystem.
      '';
    };
  ...
  config = {
    fileSystems = {
      ...
      "/" = {
        device = "/dev/disk/by-label/${config.sdImage.volumeLabel}";
        fsType = "ext4";
      };
    };
